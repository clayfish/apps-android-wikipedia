...
//        service = RetrofitFactory.newInstance(new WikiSite("www.wikidata.org", "")).create(Service.class);
        service = RetrofitFactory.newInstance(new WikiSite("www.kavitakosh.org", ""))
    ...
    @VisibleForTesting
    interface Service {
        String ACTION = "kk/api.php?action=wbgetentities&format=json&formatversion=2";

        @GET(ACTION + "&props=labels&languagefallback=1")
        @NonNull
        Call<Entities> getLabels(@Query("ids") @NonNull String idList,
